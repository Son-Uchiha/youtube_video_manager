...
Điểm cốt lõi của dự án là dùng `BullMQ + Redis` ngay từ đầu để giải bài toán đồng bộ dữ liệu theo thời gian, thay vì chỉ đọc dữ liệu tức thời một lần. Nhờ đó, hệ thống có thể chạy nhiều worker process song song trên cùng một queue, ví dụ mở nhiều terminal tab cùng chạy `npm run start:worker`, để tăng throughput xử lý khi số lượng video hoặc số lượng job tăng lên.
...
- Có thể chạy nhiều worker tab/process để xử lý nhiều job song song
...
| `Docker` | Chạy MariaDB local qua file compose hiện có |
...
- Scale đúng chỗ: có thể scale worker thay vì scale toàn bộ API server, kể cả bằng cách chạy nhiều tab/terminal worker cùng lúc trên local hoặc nhiều process khi deploy.
- Có dữ liệu nội bộ để đọc lại: hệ thống lưu snapshot metadata vào bảng `Video`, không chỉ đóng vai trò passthrough.

### Có thể tăng số lượng worker để xử lý nhanh hơn

Vì worker đọc job từ cùng queue `VIDEO` trên cùng Redis, bạn có thể chạy nhiều worker process song song để tăng throughput xử lý job. Cách đơn giản nhất ở local là mở nhiều terminal tab và chạy cùng một lệnh:
...
- số lượng video cần đồng bộ tăng lên;
- có nhiều job `video-info` chờ xử lý;
- muốn tách tải khỏi API server.

Lưu ý: nhiều worker sẽ giúp xử lý **nhiều job song song** nhanh hơn, không làm một job đơn lẻ tự nhiên chạy nhanh hơn.
...
Nếu muốn tăng throughput xử lý queue, có thể mở thêm nhiều terminal tab và chạy thêm nhiều worker process với cùng lệnh trên. Tất cả worker sẽ cùng consume từ queue `VIDEO` trên Redis.
